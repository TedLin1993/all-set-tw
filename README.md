...
| 中國信託銀行 | 存款帳戶、餘額與交易                                                           | 信用卡帳單、已入帳、未出帳與即時消費明細 | —                             | —                  | 行動銀行 API 自動登入；裝置或 OTP 驗證需先至官方 App 完成      |
...
> 玉山與國泰連接器會使用 [Cloudflare Browser Run](https://developers.cloudflare.com/browser-run/)。Workers Free Plan 每日包含 10 分鐘瀏覽器使用量；大量或頻繁同步可能超過免費額度。中國信託改由行動銀行 API 同步，不占用 Browser Run 額度。
...
中信行動銀行的 TLS endpoint 無法由 local workerd 直接連線；`npm run dev` 會自動啟動僅監聽 `127.0.0.1`、只允許 `eb.ctbcbank.com/IMP` 且使用單次隨機 token 的 Node relay，讓同步仍使用本機 D1。正式與 remote Worker 會直接連線中信，不經 relay。
